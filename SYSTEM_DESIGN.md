...
See `backend/app/api/routes/chat.py` for orchestration and mode switching, `backend/app/services/hallucination.py` for multi-signal scoring, and `backend/app/services/retrieval.py` for retrieval interface.

## 7) Deployment Strategy (Docker + Cloud)

- Containerize frontend and backend separately.
- Use managed Postgres + Redis + object storage.
- Deploy API on Kubernetes (HPA enabled), frontend on CDN/edge.
- Run workers (ingestion/eval) in async queue system (Celery/RQ).
- Use secrets manager for API keys and JWT secrets.
